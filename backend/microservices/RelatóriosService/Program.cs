...
app.MapMethods("/relatorios/gerar", methods, async context =>
{
    await ProxyRequest(context, "/agendamentos/relatorio");
});

app.MapMethods("/relatorios/tipos", new[] { "GET", "OPTIONS" }, async context =>
{
    await ProxyRequest(context, "/agendamentos/relatorio/opcoes");
});

app.MapGet("/health", () => Results.Ok(new { status = "ok", service = "relatorios-service" }));
