...
app.MapMethods("/agendamentos", methods, async context =>
{
    await ProxyRequest(context, "/agendamentos");
});

app.MapMethods("/agendamentos/{**rest}", methods, async context =>
{
    var rest = context.Request.RouteValues["rest"]?.ToString();
    var targetPath = string.IsNullOrWhiteSpace(rest) ? "/agendamentos" : $"/agendamentos/{rest}";
    await ProxyRequest(context, targetPath);
});

app.MapGet("/health", () => Results.Ok(new { status = "ok", service = "agendamentos-service" }));
